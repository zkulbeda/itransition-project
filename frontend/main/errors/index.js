...
function HomePageContent() {
  ...
  useEffect(() => {
    if (!isValidating) setSelectedUsers(selectedUsers.intersect(users.map((u) => u.ID)));
  }, [users]);

  const checkHandler = (id, e) => {
    if (selectedUsers.includes(id)) setSelectedUsers(selectedUsers.delete(id));
    else setSelectedUsers(selectedUsers.add(id));
  };

  const checkAllHandler = (e) => {
    if (isAllChecked) setSelectedUsers(selectedUsers.clear());
    else setSelectedUsers(selectedUsers.union(OrderedSet(users.map((e) => e.ID))));
  };
          ...
          <Row className="align-items-center">
            ...
            >
              <ButtonGroup>
                ...
                <Button onClick={unblockChecked} size="sm" variant="outline-primary"><UnblockIcon viewBox="0 0 24 24" /></Button>
              </ButtonGroup>
              <Button onClick={deleteChecked} size="sm" variant="danger" className="ml-1"><TrashIcon viewBox="0 0 24 24" /></Button>
            </Col>
          </Row>
        </Card.Header>
        <Card.Body>
